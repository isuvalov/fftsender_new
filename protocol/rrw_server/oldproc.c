...
int round_dist(double dist) {
  return (int)(dist+((dist-(long)dist) > 0.5));
}
...
void clr_rrw_points(target_pt_t *pts, int win_min, int win_max) {
  int i;
...
int write_to_rrw_points(target_pt_t* rrw_point, sweeps_data_t* sweeps_data, int sweep_size, int win_min, int win_max, threshold_t* threshold,
  ...
  if (i_peak_rise < 0 || i_peak_fall < 0) { //цель потеряна?
    ...
    if ((count_fail_detect < MAX_COUNT_FAIL_DETECT) && do_restore && lock) {
      ...
      memset(&rrw_point_locked, 0, sizeof(target_pt_t));
    ...
    rrw_point[harm_locked].status_direct = mvmt.mv_to_radar ? RRW_STATUS_DIRECT_TO_RADAR : RRW_STATUS_DIRECT_FROM_RADAR;
    rrw_point[harm_locked].speed = round(mvmt.speed);
...
void init_rrw_point (target_pt_t* rrw_point) {
  rrw_point->power = 0;
  rrw_point->speed = 0;
